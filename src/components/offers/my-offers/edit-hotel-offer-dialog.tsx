...
type EditHotelOfferFormData = z.infer<typeof editHotelOfferSchema>;
...
interface EditHotelOfferDialogProps {
  offer: Offer;
...
export function EditHotelOfferDialog({
  offer,
  children,
  onSubmit,
}: EditHotelOfferDialogProps) {
  ...
  const {
    ...
    setValue,
    watch,
  } = useForm<EditHotelOfferFormData>({
    resolver: zodResolver(editHotelOfferSchema),
    defaultValues: {
      name: offer.name,
      status: offer.status,
      description: offer.description || "",
      allowSplitting: offer.allowSplitting,
      rooms: offer.rooms.map((r) => ({
        ...
        groupPrice:
          typeof r === "object" && "groupPrice" in r
            ...
            : "0",
        individualPrice:
          typeof r === "object" && "individualPrice" in r && r.individualPrice
            ? r.individualPrice.toString()
            : undefined,
      })),
  ...
  const onSubmitForm = async (data: EditHotelOfferFormData) => {
    try {
      await onSubmit?.({
        name: data.name,
        status: data.status,
        ...
        invoiceFile: invoiceFile || offer.invoiceFile,
        invoiceFileName:
          invoiceFile instanceof File
            ? invoiceFile.name
            : offer.invoiceFileName,
        ...
        <form onSubmit={handleSubmit(onSubmitForm)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setValue("status", value as EditHotelOfferFormData["status"])
              }
            >
              <SelectTrigger
                id="edit-status"
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-6">
              <div className="flex items-center gap-2">
                <input
                  type="radio"
                  id="edit-hotel-bookType-soft"
                  value="soft"
                  {...register("bookType")}
                  className="size-4 cursor-pointer"
                />
                <Label
                  htmlFor="edit-hotel-bookType-soft"
              ...
              <div className="flex items-center gap-2">
                <input
                  type="radio"
                  id="edit-hotel-bookType-hard"
                  value="hard"
                  {...register("bookType")}
                  className="size-4 cursor-pointer"
                />
                <Label
                  htmlFor="edit-hotel-bookType-hard"
                  className="text-sm font-normal cursor-pointer"
                >
                  Hard Book
                </Label>
                      ...
                      >
                        ...
                        {isRoomSelected(room.id) && (
                          <div className="ml-7 grid grid-cols-2 gap-2">
                            <div>
                              <Label
                                htmlFor={`edit-group-price-${room.id}`}
                            ...
                            {allowSplitting && (
                              <div>
                                <Label
                                  htmlFor={`edit-individual-price-${room.id}`}
                                  className="text-xs text-muted-foreground"
                                >
                                  Individual Price
                                </Label>
                                <div className="flex items-center gap-1">
                                  <Input
                                    id={`edit-individual-price-${room.id}`}
                                    type="number"
                                    min="0"
                                    step="0.01"
                                    placeholder="0.00"
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="edit-contractFile">Upload Contract (PDF)</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-invoiceFile">Upload Invoice (PDF)</Label>
              <Input
                id="edit-invoiceFile"
                type="file"
                accept=".pdf,application/pdf"
                ...
                className="cursor-pointer"
              />
              {(invoiceFile || offer.invoiceFileName) && (
                <p className="text-sm text-muted-foreground">
                  {invoiceFile
                    ? `Selected: ${invoiceFile.name} (${(
                        invoiceFile.size / 1024
                      ).toFixed(2)} KB)`
                    : `Current: ${offer.invoiceFileName || "invoice.pdf"}`}
                </p>
              )}
            </div>
          </div>
